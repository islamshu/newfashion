<div class="cart-body" style="max-height: 320px; overflow-y: auto;">
                ...
                <li class="single-item border-bottom py-2">
                    <div class="item-area d-flex">
                        <div class="item-img me-2">
                            <img src="{{ Storage::url($product->thumbnails()->first()->image) }}" alt="{{ $product->name }}" width="60" height="60">
                        </div>
                        <div class="content-and-quantity flex-grow-1">
                            <div class="content">
                                ...
                                <small class="text-muted d-block">
                                    @if ($color)
                                        <span>{{ __('اللون') }}:
                                            <span style="display:inline-block;width:15px;height:15px;background:{{ $color->code }};border-radius:4px;border:1px solid #ccc;"></span>
                                        </span>
                                    @endif
                                    @if ($size)
    ...
    <div class="cart-footer p-3 border-top">
        <div class="pricing-area">
            <ul class="mb-2">
                <li class="d-flex justify-content-between"><span>{{ __('المجموع الفرعي') }}</span><span>₪{{ number_format($subTotal, 2) }}</span></li>
            </ul>
            <ul class="total fw-bold">
                <li class="d-flex justify-content-between"><span>{{ __('الإجمالي') }}</span><span>₪{{ number_format($subTotal, 2) }}</span></li>
            </ul>
        </div>
