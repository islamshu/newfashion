        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('لوحة التحكم') }}</li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <div class="tab-content" id="v-pills-tabContent">
                        <div class="tab-pane fade show active" id="v-pills-dashboard" role="tabpanel"
                            ...
                            <div class="dashboard-area box--shadow">
                                <p>{{ __('مرحبا') }}, <strong>{{ Auth::guard('client')->user()->name }} !</strong></p>
                                ...
                                <div class="row pt-25 g-4">
                                    ...
                                    <div class="col-md-4 col-sm-6">
                                        <div class="dashboard-card hover-border1 wow fadeInDown" data-wow-duration="1.5s"
                                            data-wow-delay=".6s">
                                            <div class="header">
                                                <h5>{{ __('المنتجات المفضلة') }}</h5>
                                            </div>
                                            <div class="body">
                                                <div class="counter-item">
                                                    <h2>{{ Auth::guard('client')->user()->wishlist()->count() }}</h2>
                                                </div>
                                                <div class="icon">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50"
                                                        viewBox="0 0 50 50">
                            ...
                            <div class="dashboard-profile">
                                ...
                                <div class="form-wrapper">
                                    <form id="profileForm" action="{{ route('profile.update') }}" method="POST"
                                        ...
                                        <div class="row" id="profileFields">
                                            <!-- حقول الاسم ورقم الهاتف وباقي البيانات -->
                                        ...
                                        <!-- قسم التحقق من OTP مخفي في البداية -->
                                        <div class="row" id="otpSection" style="display:none;">
                                            <div class="col-12 mb-25">
                                                <div class="form-inner">
                                                    <input type="text" name="otp" id="otp"
                                                        placeholder="{{ __('أدخل رمز التحقق المرسل إلى هاتفك') }}"
                                                        maxlength="6" />
                                                    <div class="invalid-feedback" id="error-otp"></div>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="button-group">
                                                    <button type="button" id="verifyOtpBtn"
                                                        class="primary-btn3 black-bg hover-btn5 hover-white">{{ __('تأكيد رمز التحقق') }}</button>
                                                    <button type="button" id="cancelOtpBtn"
                                                        class="primary-btn3 hover-btn5">{{ __('إلغاء') }}</button>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row" id="formButtons">
                                            <div class="col-12">
                                                <div class="button-group">
                                                    <button type="submit"
                                                        class="primary-btn3 black-bg hover-btn5 hover-white">{{ __('تحديث الملف الشخصي') }}</button>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // إرسال بيانات تحديث الملف الشخصي
            $('#profileForm').on('submit', function(e) {
                e.preventDefault();
                clearErrors();

                // بيانات الفورم بدون قسم OTP
                let data = {
                    name: $('#name').val(),
                    phone: $('#phone').val(),
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'otp_sent') {
                            ...
                            // إخفاء حقول الملف الشخصي
                            $('#profileFields, #formButtons').hide();
            ...
            $('#verifyOtpBtn').click(function() {
                ...
                if (!otpVal || otpVal.length < 6) {
                    $('#error-otp').text('الرجاء إدخال رمز التحقق كامل');
                    $('#otp').addClass('is-invalid');
                    return;
                }

                $.ajax({
                    url: "{{ route('profile.verifyOtp') }}",
                    method: 'POST',
                    data: {
                        otp: otpVal,
                        _token: $('input[name="_token"]').val()
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            // إعادة تحميل الصفحة أو تحديث الحقول لعرض الرقم الجديد
                            setTimeout(() => location.reload(), 1500);
                        }
                    },
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            $('#error-otp').text(xhr.responseJSON.message ||
                                'رمز التحقق غير صحيح.');
                            $('#otp').addClass('is-invalid');
                        } else {
                            Swal.fire({
            ...
            // زر إلغاء التحقق من OTP (يعيد عرض الفورم)
            $('#cancelOtpBtn').click(function() {
                $('#otpSection').hide();
                $('#profileFields, #formButtons').show();
                clearErrors();
            });

            // زر إلغاء تعديل الملف الشخصي (يعيد تعيين الفورم)
