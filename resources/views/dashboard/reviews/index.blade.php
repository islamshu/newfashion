            ...
            <tbody>
                @forelse ($reviews as $review)
                    <tr>
                        ...
                        <td>
                            @if($review->image)
                                <img src="{{ asset('storage/' . $review->image) }}" width="60" height="60" class="rounded-circle" alt="image">
                            @else
                        ...
                        <td>
                            <a href="{{ route('reviews.edit', $review->id) }}" class="btn btn-sm btn-info">
                                <i class="fa fa-edit"></i> {{ __('تعديل') }}
                            </a>
                            <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                                <button onclick="return confirm('{{ __('هل أنت متأكد؟') }}')" class="btn btn-sm btn-danger">
